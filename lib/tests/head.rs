...
mod tests {
    ...
    fn head_builder_output_are_consistent() {
        ...
        let head1 = HeadBuilder::new( doc_start1, metadata_start1, hash_algos.clone(), signature_algo.clone()).build();
        let head2 = HeadBuilder::new( doc_start2, metadata_start2, hash_algos.clone(), signature_algo.clone()).build();


        assert_eq!(head1.document_start + (doc_start2 - doc_start1), head2.document_start);
        assert_eq!(head1.metadata_start + (doc_start2 - doc_start1) + (metadata_start2 - metadata_start1), head2.metadata_start);
